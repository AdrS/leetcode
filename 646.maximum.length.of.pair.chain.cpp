...
public:
    int findLongestChain(vector<vector<int>>& pairs) {
        ...
        // OPT(i) = length of longest chain ending with pairs[i]
        // OPT(0) = 1
        // OPT(i) = max(1, max for 0 <= j < i where pairs[j].end < pairs[i].start of OPT(j) + 1)
        // O(n^2) memory
        // O(n) space
        vector<int> longest;
        for(unsigned i = 0; i < pairs.size(); ++i) {
            int best = 1;
            // for each possible predecessor in chain
